const createSprintsService = ({ allAsync, getAsync, runAsync }) => {
  ...
  const listSprints = (userId, { archived = 0 } = {}) =>
    allAsync(
      `${sprintSelect}
     WHERE s.archived = ?
  ...
  const getSprintForUser = (id, userId) =>
    getAsync(
      `${sprintSelect}
  ...
  const getOwnedSprintForUser = (id, userId) =>
    getAsync(
      `${sprintSelect}
  ...
  const replaceSprintEditors = async ({ sprintId, editorUserIds = [] }) => {
    const normalizedEditorUserIds = [
      ...new Set(editorUserIds.map((id) => Number(id)).filter((id) => Number.isInteger(id) && id > 0)),
    ];
