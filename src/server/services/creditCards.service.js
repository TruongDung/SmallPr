const CREDIT_CARD_SELECT =
  'id, name, card_user, issuer, total_balance, interest_charge, closing_date, created_at, updated_at';
...
const createCreditCardsService = ({ allAsync, getAsync, runAsync }) => {
  ...
  const findForUser = (id, userId) => getAsync('SELECT * FROM credit_cards WHERE id = ? AND user_id = ?', [id, userId]);

  const findById = (id) => getAsync(`SELECT ${CREDIT_CARD_SELECT} FROM credit_cards WHERE id = ?`, [id]);

  const getAccountUser = (userId) => getAsync('SELECT username FROM users WHERE id = ?', [userId]);

  const listCardUsersForUser = (userId) =>
    allAsync(
      `SELECT name
     FROM (
  ...
  const remove = ({ id, userId }) =>
    runAsync('DELETE FROM credit_cards WHERE id = ? AND user_id = ? RETURNING id', [id, userId]);

  const listFastAccessBillsForUser = (userId) =>
    allAsync(
  ...
  const listFastAccessLinksForUser = (userId) =>
    allAsync(
  ...
  const createFastAccessLink = async ({ userId, label, url }) => {
    ...
    return getAsync(`SELECT ${FAST_ACCESS_LINK_SELECT} FROM fast_access_links WHERE id = ?`, [result.lastID]);
  };

  const findFastAccessLinkForUser = (id, userId) =>
    getAsync(`SELECT ${FAST_ACCESS_LINK_SELECT} FROM fast_access_links WHERE id = ? AND user_id = ?`, [id, userId]);

  const removeFastAccessLink = ({ id, userId }) =>
    runAsync('DELETE FROM fast_access_links WHERE id = ? AND user_id = ? RETURNING id', [id, userId]);
  ...
  const seedFastAccessBillsForUser = async (userId) => {
    const existing = await getAsync('SELECT id FROM fast_access_bills WHERE user_id = ? LIMIT 1', [userId]);
    if (existing) return;
  ...
  const findFastAccessBillForUser = (id, userId) =>
    getAsync('SELECT * FROM fast_access_bills WHERE id = ? AND user_id = ?', [id, userId]);

  const findFastAccessBillById = (id) =>
    getAsync(`SELECT ${FAST_ACCESS_BILL_SELECT} FROM fast_access_bills WHERE id = ?`, [id]);
  ...
  const removeFastAccessBill = (id, userId) =>
    runAsync('DELETE FROM fast_access_bills WHERE id = ? AND user_id = ?', [id, userId]);
