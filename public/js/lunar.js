// Lunar calendar converter: Gregorian <-> Lunar (Chinese lunar calendar)
// Based on lookup tables and algorithms for lunar new year calculations.
// Returns { lunar_year, lunar_month, lunar_day } for a given Gregorian date.
(function () {
  'use strict';

  // Lookup table for lunar new year dates (first day of lunar year 1900–2100)
  ...
  // Days in each lunar month (non-leap years)
  const daysInLunarMonth = [30, 29, 30, 29, 30, 29, 30, 29, 30, 29, 30, 29];

  // Days in each lunar month (leap years — intercalary month at position 6)
  const daysInLunarMonthLeap = [30, 29, 30, 29, 30, 29, 30, 30, 30, 29, 30, 29, 30];

  // Leap year flags: 1 = leap (has intercalary month), 0 = normal
  const leapYears = {
  ...
  // Input: Date object (Gregorian)
  // Output: { lunar_year, lunar_month, lunar_day }
  function toHarmonic(date) {
    ...
    // Find the lunar year that contains this date
    let lunar_year = gy;
    const nyStart = lunarNewYearDates[lunar_year];
    if (!nyStart) return null;

    const [ny_month, ny_day] = nyStart;

    // If the date is before lunar new year, it belongs to previous lunar year
    const isBeforeNY = gm < ny_month || (gm === ny_month && gd < ny_day);
    if (isBeforeNY) lunar_year--;

    // Create date object for lunar year start
    const nyDate = new Date(lunar_year, ny_month - 1, ny_day);
    const targetDate = new Date(gy, gm - 1, gd);

    // Calculate days from lunar year start
    let daysFromStart = Math.floor((targetDate - nyDate) / 86400000) + 1;

    // If negative, we need to go back one lunar year
    if (daysFromStart < 1) {
      lunar_year--;
      const prevNyStart = lunarNewYearDates[lunar_year];
      if (!prevNyStart) return null;
      const prevNyDate = new Date(lunar_year, prevNyStart[0] - 1, prevNyStart[1]);
      daysFromStart = Math.floor((targetDate - prevNyDate) / 86400000) + 1;
    }

    // Map days to lunar month and day
    const isLeap = leapYears[lunar_year] === 1;
    const monthDays = isLeap ? daysInLunarMonthLeap : daysInLunarMonth;

    let lunar_month = 1;
    let lunar_day = daysFromStart;
  ...
  // Check if this is the 1st or 15th day of lunar month
  function isLunarMarked(date) {
    const lunar = toHarmonic(date);
    if (!lunar) return false;
    return lunar.lunar_day === 1 || lunar.lunar_day === 15;
  }

  // Get lunar day info for display
  function getLunarDay(date) {
    ...
    return {
      ...
      isFirstDay: lunar.lunar_day === 1,
      isFullMoon: lunar.lunar_day === 15,
    };
  }
